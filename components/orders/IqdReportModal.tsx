...
import { X, Truck, CheckCircle2, WalletCardsIcon } from "lucide-react";
import Cookies from "js-cookie";
import { Order } from "@prisma/client";
import { useApp } from "@/providers/AppProvider";
import { motion, AnimatePresence } from "framer-motion";

interface IqdReportModalProps {
  isOpen: boolean;
  onClose: () => void;
  order: Order;
...
export default function IqdReportModal({
  isOpen,
  onClose,
  order,
}: IqdReportModalProps) {
  ...
  const [formData, setFormData] = useState({
    iqdPrice: order.iqdPrice ? order.iqdPrice.toString() : "",
    iqdShippingPrice: order.iqdShippingPrice
      ? order.iqdShippingPrice.toString()
      : "",
  });
      ...
      const response = await fetch(`/api/orders/${order.id}`, {
        ...
        body: JSON.stringify({
          iqdPrice:
            formData.iqdPrice === ""
              ? 0
              : Number(parseFloat(formData.iqdPrice)),
          iqdShippingPrice:
            formData.iqdShippingPrice === ""
              ? 0
              : Number(parseFloat(formData.iqdShippingPrice)),
        }),
      });
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      >
                        ...
                        <div className="relative">
                          ...
                          <input
                            type="text"
                            name="iqdShippingPrice"
                            value={formData.iqdShippingPrice}
                            onChange={handleInputChange}
                            className="w-full pl-10 pr-4 py-2.5 text-sm rounded-xl bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-600 focus:border-primary dark:focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all duration-200"
                            placeholder="0.00"
                          />
